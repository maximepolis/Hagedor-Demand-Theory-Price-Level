%% Figure 10: Phase diagram (without inflation)


temp=results_noi.s;
results_noi.s_phase=nan(size(a));
for it1=1:size(temp,1)
    for it2=2:size(temp,2)
        if temp(it1,it2)*temp(it1,it2-1)<=0
            results_noi.s_phase(it1)=(z(it2)*temp(it1,it2-1)-z(it2-1)*temp(it1,it2))/(-temp(it1,it2)+temp(it1,it2-1));
        end
    end
end
...
hold on
plot(a,exp(results_noi.s_phase),'-','Color',[0.8,0.5,0.0],'linewidth',2)
plot(a,ones(size(a)),'-','Color',[0.3,0.7,0.5],'linewidth',2)
plot(a,exp(results_noi.def),'-','Color',[0.3,0.5,0.8],'linewidth',2)
plot(-sss_noi.a,exp(sss_noi.z),'*','Color',[0.9,0.8,0.2],'linewidth',2)
...
print -dpdf    p90_phase_noi
savefig(myfigPD,'g90_phase_noi.fig');
