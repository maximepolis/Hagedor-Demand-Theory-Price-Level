...
% Computes the HJB without inflation

function results = f1_HJB_NoInflation(parameters)
...
for m=1:maxitD
    ...
    for n=1:maxit
        ...
        if mod(n,100)==0
            log__=pad([ num2str(m) '-noi  d_it: ' num2str(m) '  HJB_it: ' num2str(n) '     dist V: ' num2str(dist1(mn)) '     dist Q: ' num2str(dist2(mn)) '     dist V_def: ' num2str(dist3(mn)) '     dist Q_def: ' num2str(dist4(mn))],175);
            disp(log__)
            log_=[log_;log__];
        end
