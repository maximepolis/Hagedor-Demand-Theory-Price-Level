...
%   R2 CARBON    carbon-tax-style proportional levy funds the program:
%                tau_ls = r*b,               vartheta = g/(1-D)
%   R3 REBATE    levy at twice the program, half rebated lump-sum
%                (Kaenzig-style progressive rebate design):
%                tau_ls = r*b - g,           vartheta = 2g/(1-D)
%   R4 MIXED     half deficit / half levy:
...
REG{1} = struct('name','R1-DEFICIT','Bnom',B, 'g',g_of, 'D',D_of, ...
    ...
    'vartheta', @(P) 0);
REG{2} = struct('name','R2-CARBON','Bnom',B, 'g',g_of, 'D',D_of, ...
    'tau_ls',   @(P) rb_of(P), ...
    'vartheta', @(P) g_of(P) ./ (1 - D_of(P)));
REG{3} = struct('name','R3-REBATE','Bnom',B, 'g',g_of, 'D',D_of, ...
    'tau_ls',   @(P) rb_of(P) - g_of(P), ...
    'vartheta', @(P) 2 * g_of(P) ./ (1 - D_of(P)));
REG{4} = struct('name','R4-MIXED','Bnom',B, 'g',g_of, 'D',D_of, ...
...
if fid > 0
    ...
    for k = 1:numel(RREG)
        fprintf(fid, ['%-11s P*=%.4f D=%.4f tau_ls=%+.4f vth=%.4f ' ...
...
for k = 1:numel(RREG)
    fprintf(' %-11s P*=%.4f  nu=%.3f  bottom50 %+.2f%%  top10 %+.2f%%\n', ...
