...
| 18 | Deficit-ratchet reversal point: illustrative extension | 20 | **=**, we are more specific | **IE** as a joint result; **WP** as a *tax-timing* object. The exercise confounds delayed taxation with a permanent terminal-debt ratchet, so the referee's "illustrative" is right but understates why. | tex 3296–3300 states in the manuscript's own words that the matched-terminal-debt path "is not computed"; R10 row 20 records the applied edits (caveat precedes the number; macros carry a `Ratchet` suffix) | The matched-terminal-debt path. The machinery exists: `src_project/kv_solve_consolidation.m` solves the surcharge amplitude $\xi_t=a_\xi h_t$; drivers `main_transition_deficit_ladder.m` / `main_deficit_decomposition.m`. Equal-PV and equal-terminal-real cases are still to be added |
...
Nothing moves *upward* out of **WP**. No two-asset status changed.
