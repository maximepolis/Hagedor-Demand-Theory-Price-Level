...
| `green_hank2.mod` + `run_green_hank2.m` | U7 tier 1b HANK (two-asset) | liquid nominal bonds vs illiquid equity/capital (KMV/ABRS structure, from the verified `hank_two_assets` example), sticky wages+prices, Tobin-Q investment, ENDOGENOUS government debt with the PHIB financing-speed margin, climate block on TFP; four regimes, PFig17. **CLOSED THIS ROUND: accuracy protocol attempted 2026-07-07 and failed at the solver level on Dynare 8-unstable (singular Jacobian / hard child crash); NOT REPORTABLE; the driver fail-fasts after 2 consecutive failures and writes `hank2_protocol_verdict.txt`. Revisit on a stable Dynare release** |
