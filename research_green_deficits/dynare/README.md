...
| `green_hank2.mod` + `run_green_hank2.m` | U7 tier 1b HANK (two-asset) | liquid nominal bonds vs illiquid equity/capital (KMV/ABRS structure, from the verified `hank_two_assets` example), sticky wages+prices, Tobin-Q investment, ENDOGENOUS government debt with the PHIB financing-speed margin, climate block on TFP; four regimes, PFig17. **RCOND=NaN mechanism FOUND AND FIXED (2026-07-07): the template's sign()/abs()^(chi2-1) forms produce 0×Inf=NaN in symbolic second derivatives at the illiquid-constraint corner, inside the sequence-space Jacobian; the .mod now carries the exact smooth chi2=2 polynomial equivalents + drops the only exo-lead auxiliary. Fix UNVERIFIED — driver kill-switched by default; test with `TIER1B_FORCE = true; run_green_hank2`. NOT REPORTABLE until the protocol passes** |
