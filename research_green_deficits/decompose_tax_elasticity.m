...
% to above-mean. NOTE: the measured stationary rise in S operates through
% the endogenous distribution, NOT the direct fixed-state saving response
% (part (e): the direct covariance term is ~0 and slightly negative).
...
if have_cov
    ...
    fprintf('  direct term (per rev) %+.3f;  full stationary tilt %+.3f;  implied distribution term %+.3f\n', ...
            cov_perRev, eps_tilt, cov_shift_resid);
    if sign(cov_perRev) ~= sign(eps_tilt)
        fprintf(['  READING: the direct fixed-state covariance term has the OPPOSITE sign\n' ...
                 '  from the full stationary tilt response. The positive long-run response\n' ...
                 '  is generated by the endogenous distributional adjustment, not by the\n' ...
                 '  direct covariance term. Do not attribute the sign to the covariance;\n' ...
                 '  audit the residual decomposition (audit_tax_incidence) before claiming\n' ...
                 '  more than direct-vs-distributional.\n']);
    else
        fprintf('  READING: direct covariance term and full tilt response share a sign.\n');
    end
else
