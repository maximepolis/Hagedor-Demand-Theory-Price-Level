...
if exist(bf,'file') ~= 2
    ...
    tee(['***\n*** The two-way comparison this file used to run compared two branches\n' ...
         ...
         '***\n*** EXPECTED AFTER A FRESH EXTRACT, not a defect: output/baseline\n' ...
         '*** is a locally built artifact and no download restores it (see\n' ...
         '*** EXTRACT_RULE.md -- results flow this machine -> repo, never back).\n' ...
         '*** One capture run writes BOTH legs, d10 and d11, so it is one\n' ...
         '*** command before the pair of parity checks, once per extract.\n']);
    fclose(fid);
