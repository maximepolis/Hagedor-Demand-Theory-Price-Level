...
| Climate-risk shock process (aggregate) | **Stage A RUN VERIFIED 2026-07-09, in the paper** | two-regime (Calm/Severe) DTPL: the nominal bond nominally safe but really risky, two state-contingent price levels clearing the safe-asset market state by state, green investment compressing the systematic climate risk. Code: `solve_household_vfi_agg.m` (VFI over (a,e,s), state-contingent return, interpolated continuation) + `stat_dist_agg.m` (exact joint dist) + `solve_dtpl_aggrisk.m` (two-price fixed point + premium diagnostics) + `main_project_aggrisk.m` (PFig19, aggrisk_summary.txt). MATLAB run VERIFIED (na=100: baseline price dispersion 0.367, Calm->Severe disaster real return -25.4%, premium +0.43%; a 2%-of-income green program compresses the dispersion to 0.267 and the disaster loss to -19.5%) and matches the Python prototype. In the paper as Section subsec:aggrisk + Result 7 + PFig19. STOCHASTIC AGGREGATE RISK tier; Stages B/C (nu_aggrisk decomposition, welfare, stochastic transition) in `appendix/AGGREGATE_RISK_PLAN.md` |
