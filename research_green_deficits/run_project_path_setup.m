...
% OUTPUT  INFO .rootdir .projdir .added .collisions .winner .ok
    ...
    % THE FROZEN BASELINE MUST NEVER BE ON THE MAIN PATH. baseline_bf0a4e8/
    % contains a pre-refactor namesake for all 152 files, including
    % main_twoasset_ownership_kv with its ten local functions. If it were
    % reachable here, a current driver could resolve to pre-refactor code with
    % no error and no sign that it had -- the worst version of the shadowing
    % problem this function exists to prevent. It sits outside both source
    % trees so genpath cannot reach it; this checks rather than assumes.
    onpath = strsplit(path, pathsep);
    leaked = onpath(contains(onpath, 'baseline_bf0a4e8', 'IgnoreCase', true));
    if ~isempty(leaked)
        error('run_project_path_setup:baselineLeak', ...
            ['the frozen pre-refactor baseline is on the MATLAB path (%s). ' ...
             'It must never be: its files would shadow current code silently. ' ...
             'Run  restoredefaultpath; run_project_path_setup  again -- ' ...
             'main_baseline_capture restores the path itself, so this means a ' ...
             'capture was interrupted.'], leaked{1});
    end
