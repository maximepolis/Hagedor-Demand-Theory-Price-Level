...
| U4 | Financing regimes — **machinery DONE** (`solve_regime_equilibrium.m` exact solver with proportional levy; `main_project_regimes.m`: deficit / carbon-levy / rebate / mixed at the calibrated medium column, PFig9; identical aggregate budgets so differences are pure incidence + price level). Numbers pending run. Remaining within U4: distortionary labor tax with an hours margin (Barrage template) — the levy's Pigouvian margin belongs to U8 | run pending | — |
