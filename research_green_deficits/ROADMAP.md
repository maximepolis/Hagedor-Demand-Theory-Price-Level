...
| U7 | HANK transitions — **PLAN WRITTEN** (appendix/HANK_TRANSITION_PLAN.md: states, unknowns, Jacobians, residuals, terminal conditions, experiments, validation); implementation NOT YET STARTED. Preferred route now: Dynare's native heterogeneity framework (`heterogeneity_dimension` + `heterogeneity_solve`, sequence-space Jacobians), following the user-supplied examples in `heterogeneity/` at repo root | weeks | U6 first |
