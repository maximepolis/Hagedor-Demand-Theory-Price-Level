...
| U3 | ~~Calibration execution~~ **DONE for beta / program scale / damage columns** (beta*=0.9296 to debt/GDP=1.10; program 2% of income; nu = 0.15 / 0.56 / 2.01 across LOW/MED/HIGH damages — verified run, 147 s). The theta_g point-calibration item is SUPERSEDED by S11: no published estimate anchors abatement effectiveness, so theta_g is reported as an AXIS — the nu=1 frontier over the (D0,theta_g) plane — rather than a point value. Remaining within U3: psi from Känzig incidence; wealth-moment targeting of (rho, sig_eps) | partial | data access |
...
| U7 | HANK transitions — **TIER 1 DONE incl. TAYLORBAL, runs VERIFIED and in the paper** (five regimes; TAYLOR-vs-TAYLORBAL: deficit financing buys the debt path (+0.071 vs +0.007 at 40q), not the inflation path (+0.44% vs +0.36%); verified RHOG=0.98 master run 2026-07-06). **TIER 1b NOW SOLVES (run 6, 2026-07-07)** (`green_hank2.mod` + `run_green_hank2.m`: two-asset HANK): the chi2=2 smoothing fix (which removed the reproducible sequence-space Jacobian NaN from sign()/abs()^(chi2-1) kink derivatives at the illiquid-constraint corner) plus the base-workspace IRF-harvest fix in `solve_hank_regime_batch.m` took the tier all the way through — all four regimes (WEAK/TAYLOR/GREENACCOM/TAYLORBAL) solve, produce finite non-divergent IRFs (horizon 200), and CLEAR the oscillation diagnostic (scores 1); PFig17 + hank2_irfs_summary.txt + hank2_validation.txt written (~70 min run). OPT-IN by default (TIER1B_FORCE=true) as a long experimental solve. **REFINEMENT PASSED (2026-07-09: THORIZON 400, na 33, SS tol 1e-8 vs baseline 400/15/30/1e-10; max rel. dev 0.067 < 0.10 gate) — magnitudes NOW REPORTED in the paper** (Table "twoasset" + PFig17): rule-independent green path (kg +0.165 all regimes), deficit buys the debt path not the inflation path (+0.077 vs +0.008 at 40q; pi +0.04% vs +0.03%), liquid return mirrors inflation (the B/P Fisher margin), equity reprices −5 to −7%. **TIER 2 RUN VERIFIED & REPORTABLE, grid-consistent (na=500, T=80, 2026-07-07 rerun)** (`solve_hank_dtpl_transition.m` + `main_project_transition.m`, PFig18: price-level path clears the asset market at every date, backward/forward exact HA solution, Anderson-accelerated fixed point, interior resid <5e-4, terminal <5e-4, beta calibrated on this run's own na=500 grid so pre-announcement debt/GDP lands on the 1.10 target): impact disinflation −2.0%/yr (−3.9% price-level jump = 77% of the long-run −5.1% front-loaded at announcement), bondholder windfall 5.3% of program PV (both designs), back within a quarter-point of trend by year 2, nominal-vs-indexed near-identical on-path (insulation value is off-equilibrium) — the paper's dynamic centerpiece, in Section tier2 + Result 6 + abstract. Both tier-1b accuracy gates now PASS and the two-asset results are in the paper | — | — |
...
| S11 | Calibration robustness — **IMPLEMENTED, run PENDING on the user machine** (`main_project_robustness.m`, U9-adjacent): (a) nu over the (D0,theta_g) plane at the calibrated scale with the nu=1 frontier (D0 anchored at the three sourced columns 0.02/0.06/0.20 + two interpolating nodes; theta_g swept 0–3), sign(nu_reval) recorded at every node; (b) one-at-a-time sensitivity at MEDIUM under the HELD-DEBT-TARGET protocol (sigma {1,3}, phi_D {0,1}, psi {1,2} re-calibrate beta; delta_g 0.05 and scale {1%,4%} hold beta*). Outputs PFig20 + robustness_summary.txt; paper subsection "Robustness: what one must believe..." (subsec:robustness) written with Table "sources"; quantitative sentences GATED on the run | run pending | U3 |
