...
| U6 | RANK transitions — **machinery DONE** (external steady-state file; four monetary regimes incl. the temporary green-accommodation rule tied to the kg gap; `run_green_transitions.m` driver, PFig13). Run pending (requires Dynare on the user machine) | run pending | Dynare |
| U7 | HANK transitions — **PLAN WRITTEN** (appendix/HANK_TRANSITION_PLAN.md: states, unknowns, Jacobians, residuals, terminal conditions, experiments, validation); implementation NOT YET STARTED | weeks | U6 first |
