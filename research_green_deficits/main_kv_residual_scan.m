...
% ---------------------------------------- the base the factors are relative to
% THE SCAN MUST START FROM THE CALIBRATION BASE. Its entire output is a pair
% of factors (kfac, bfac) that two other drivers read from
% kv_residual_scan.mat and apply to the calibration; a factor is meaningful
% only relative to a base, so if this run starts from an already-widened grid
% the numbers it publishes denote a grid nobody else will build.
%
% That is not hypothetical. main_twoasset_ownership_kv with REGRID = true
% saves the WIDENED p into twoasset_ownership_kv.mat, which is the file
% loaded above. Running the scan after a REGRID run therefore probes
% kfac x (whatever is already there) and reports the first factor alone. Stop
% instead, and say which file to rebuild.
GS0 = kv_grid_state(CTX.p);
tee('loaded grid: kmax %.1f, bmax %.2f -> kfac %.4g, bfac %.4g (%s)\n', ...
    GS0.kmax, GS0.bmax, GS0.kfac, GS0.bfac, GS0.source);
if ~NOWIDEN && ~(abs(GS0.kfac-1) < 1e-8 && abs(GS0.bfac-1) < 1e-8)
    tee(['\n*** REFUSING TO SCAN. The loaded calibration is already widened\n' ...
         '*** (kfac %.4g, bfac %.4g). The factors this driver publishes are read\n' ...
         '*** by main_preferred_decomposition and main_preferred_signal_noise and\n' ...
         '*** applied to the calibration, so they must be relative to the\n' ...
         '*** calibration base (kmax %.0f, bmax %.0f), not to an already-widened\n' ...
         '*** grid. Re-run  clear; main_twoasset_ownership_kv  with REGRID = false\n' ...
         '*** to restore a base-grid calibration, or pass NOWIDEN = true to scan\n' ...
         '*** the widened grid as-is without republishing factors.\n'], ...
        GS0.kfac, GS0.bfac, GS0.base.kmax, GS0.base.bmax);
    fclose(fid);
    error('main_kv_residual_scan:alreadyWidened', ...
        'loaded grid is at kfac=%.6g bfac=%.6g; the scan must start from the base', ...
        GS0.kfac, GS0.bfac);
end
