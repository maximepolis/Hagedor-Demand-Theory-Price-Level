...
if strcmp(Policy,'on')==1
    if policy_counter == 1
        matrix.policy =  lambda_pol(temperature.mesh,state.mesh,1,2).*pdr1_s1(:,:,2,tipping_counter,breakthrough_counter).*(1+j_hat_D1z2(:,:,1,tipping_counter,breakthrough_counter))...
           + lambda_pol(temperature.mesh,state.mesh,1,3).*pdr1_s1(:,:,3,tipping_counter,breakthrough_counter).*(1+j_hat_D1z3(:,:,1,tipping_counter,breakthrough_counter));
    ...
    elseif policy_counter == 2
        matrix.policy = lambda_pol(temperature.mesh,state.mesh,2,1).*pdr1_s1(:,:,1,tipping_counter,breakthrough_counter).*(1+j_hat_D1z1(:,:,2,tipping_counter,breakthrough_counter))...
           + lambda_pol(temperature.mesh,state.mesh,2,3).*pdr1_s1(:,:,3,tipping_counter,breakthrough_counter).*(1+j_hat_D1z3(:,:,2,tipping_counter,breakthrough_counter));
    ...
    elseif policy_counter == 3
        matrix.policy =  lambda_pol(temperature.mesh,state.mesh,3,2).*pdr1_s1(:,:,2,tipping_counter,breakthrough_counter).*(1+j_hat_D1z2(:,:,3,tipping_counter,breakthrough_counter))...
           + lambda_pol(temperature.mesh,state.mesh,3,1).*pdr1_s1(:,:,1,tipping_counter,breakthrough_counter).*(1+j_hat_D1z1(:,:,3,tipping_counter,breakthrough_counter));
...
if strcmp(Tipping,'on')==1
    if tipping_counter == 1
        matrix.tipping = lambda_tipp(temperature.mesh,state.mesh,policy_counter,tipping_counter).*pdr1_s1(:,:,policy_counter,2,breakthrough_counter).*(1+j_hat_D1x2(:,:,policy_counter,1,breakthrough_counter))...
                        +lambda_tipp(temperature.mesh,state.mesh,policy_counter,tipping_counter).*pdr1_s1(:,:,policy_counter,3,breakthrough_counter).*(1+j_hat_D1x3(:,:,policy_counter,1,breakthrough_counter));
        lambda_tipp_sum = 2*lambda_tipp(temperature.mesh,state.mesh,policy_counter,tipping_counter);
    elseif tipping_counter == 2
        matrix.tipping = lambda_tipp(temperature.mesh,state.mesh,policy_counter,tipping_counter).*pdr1_s1(:,:,policy_counter,3,breakthrough_counter).*(1+j_hat_D1x(:,:,policy_counter,2,breakthrough_counter));
...
if breakthrough_counter < breakthrough.number+1
    matrix.breakthrough = lambda_breakthrough(temperature.mesh,state.mesh,policy_counter,tipping_counter,breakthrough_counter).*pdr1_s1(:,:,policy_counter,tipping_counter,breakthrough_counter+1).*(1+j_hat_D1b(:,:,policy_counter,tipping_counter,breakthrough_counter));
else
    matrix.breakthrough = 0;
end

%% Create Coefficients
matrix.K_1 = 1 + matrix.policy + matrix.tipping + matrix.breakthrough;

matrix.K_2 = Hat_D1.mu(:,:,policy_counter,tipping_counter,breakthrough_counter)...
        + intensity.lambda.*(-jump.moment(leverage.phi-pref.gamma))...
        + climate.lambda.*(-climate.moment(leverage.phi-pref.gamma))...
        - lambda_pol_sum...
        - lambda_tipp_sum...
        - lambda_breakthrough(temperature.mesh,state.mesh,policy_counter,tipping_counter,breakthrough_counter);  
    
matrix.K_3 = state.mesh.*(1-state.mesh).*state.drift...
        + state.mesh.*(1-state.mesh).*(production.sigma(2)*production.rho12-production.sigma(1)).*Hat_D1.sigma1(:,:,policy_counter,tipping_counter,breakthrough_counter)...
        + state.mesh.*(1-state.mesh).*production.sigma(2).*sqrt(1-production.rho12^2).*Hat_D1.sigma2(:,:,policy_counter,tipping_counter,breakthrough_counter);              

matrix.K_4 = 0.5.*state.variation.*state.mesh(:,:).^2.*(1-state.mesh).^2;                                                                 

matrix.K_5 = temperature.drift + temperature.mesh.*temperature.sigma .*Hat_D1.sigma3(:,:,policy_counter,tipping_counter,breakthrough_counter);
...
Vector_RHS = pdr1_s1Old(:,:,policy_counter,tipping_counter,breakthrough_counter)./time.delta + matrix.K_1; 
