...
if strcmp(transition,'off')==1
    ...
    if policy_counter==1 && policy_target==2
        lambda_pol=0*lambda12.*exp(mu_plus*max(T-1.5,0)-nu_plus*S);
    elseif policy_counter==2 && policy_target==3
        lambda_pol=0*lambda23*exp(mu_plus*max(T-1.5,0)-nu_plus*S);
    elseif policy_counter==1 && policy_target==3
        lambda_pol=0*lambda13*exp(mu_plus*max(T-1.5,0)-nu_plus*S);
        lambda_pol=0.0361;
    elseif policy_counter==3 && policy_target==1
        lambda_pol=0*lambda31./exp(mu_minus*max(T-1.5,0)-nu_minus*S);
    elseif policy_counter==2 && policy_target==1
        lambda_pol=0*lambda21./exp(mu_minus*max(T-1.5,0)-nu_minus*S);
    elseif policy_counter==3 && policy_target==2
        lambda_pol=0*lambda32./exp(mu_minus*max(T-1.5,0)-nu_minus*S);
    else
        lambda_pol=0;
    end
end
