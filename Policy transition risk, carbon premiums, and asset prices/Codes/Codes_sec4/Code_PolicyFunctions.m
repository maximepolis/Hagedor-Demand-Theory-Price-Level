...
for breakthrough_counter=breakthrough.number+1:-1:1
    for policy_counter=policy.number+1:-1:1
        for tipping_counter=tipping.number+1:-1:1
            ...
            color.green=[0.75 0.75 0.75];
            color.brown=[0.25, 0.25, 0.25];
            color.middle=(color.brown+color.green)/2;

            subplot(3,3,1)
            p1=plot(x_axis, policy2015.c);
            title('a) Consumption Rate, C / Y', 'FontWeight', 'normal');
            set(p1(1),'Color', color.green, 'LineStyle','-', 'LineWidth', width);
            set(p1(2),'Color', color.middle, 'LineStyle','-', 'LineWidth', width);
            set(p1(3),'Color', color.brown, 'LineStyle','-', 'LineWidth', width);
  ...
  set(p1(1),'Color', color.green, 'LineStyle','-', 'LineWidth', width);
        ...
        set(p1(1),'Color', color.green, 'LineStyle','-', 'LineWidth', width);
            ...
            subplot(3,3,4)
            p1=plot(x_axis, policy2015.f2*10^15);
            title('d) Brown Sector: Fossil Fuel, f_2', 'FontWeight', 'normal');
           set(p1(1),'Color', color.green, 'LineStyle','-', 'LineWidth', width);
            set(p1(2),'Color', color.middle, 'LineStyle','-', 'LineWidth', width);
            set(p1(3),'Color', color.brown, 'LineStyle','-', 'LineWidth', width);
            axis([0 4 min(min(policy2015.f2*10^15))*0.95 max(max(policy2015.f2*10^15))*1.05+10^(-15)])
            subplot(3,3,5)
            p1=plot(x_axis, policy2015.g2*10^15);
            title('e) Brown Sector: Renewables, g_2', 'FontWeight', 'normal');
            set(p1(1),'Color', color.green, 'LineStyle','-', 'LineWidth', width);
            set(p1(2),'Color', color.middle, 'LineStyle','-', 'LineWidth', width);
            set(p1(3),'Color', color.brown, 'LineStyle','-', 'LineWidth', width);
            axis([0 4 min(min(policy2015.g2*10^15))*0.95 max(max(policy2015.g2*10^15))*1.05+10^(-15)])
            subplot(3,3,6)
            p1=plot(x_axis, policy2015.g1*10^15);
            title('f) Green Sector: Renewables, g_1', 'FontWeight', 'normal');
            set(p1(1),'Color', color.green, 'LineStyle','-', 'LineWidth', width);
            set(p1(2),'Color', color.middle, 'LineStyle','-', 'LineWidth', width);
            set(p1(3),'Color', color.brown, 'LineStyle','-', 'LineWidth', width);
            axis([0 4 min(min(policy2015.g1*10^15))*0.95 max(max(policy2015.g1*10^15))*1.05+10^(-15)])
            subplot(3,3,8)
...
time.plot=1;
policy.DeltaCP=DeltaCP(entry_s,entry_t)';
policy.DeltaRf=DeltaRf(entry_s,entry_t)';
policy.DeltaP1=DeltaP1(entry_s,entry_t)';
policy.DeltaP2=DeltaP2(entry_s,entry_t)';
policy.DeltaPDR1=DeltaPDR1(entry_s,entry_t)';
policy.DeltaPDR2=DeltaPDR2(entry_s,entry_t)';
 % Graphen
            graph.priceimpact=figure(10);
            set(graph.priceimpact,'Units','Pixels','Position',[0 0 800 400]);
            % Limits of the x-axis.
            xlim_ = [Inf -Inf];
            % Axes for the plot.
            ax_ = axes;
            ...
            subplot(2,3,1)
            p1=plot(x_axis, policy.DeltaP1*100);
            title('a) Price Impact Green Asset [%]', 'FontWeight', 'normal');
             set(p1(1),'Color', color.green, 'LineStyle','-', 'LineWidth', width);
            set(p1(2),'Color', color.middle, 'LineStyle','-', 'LineWidth', width);
            set(p1(3),'Color', color.brown, 'LineStyle','-', 'LineWidth', width);
          %  axis([0 4 min(min(policy2015.c))*0.95 max(max(policy2015.c))*1.05])
            subplot(2,3,2)
            p1=plot(x_axis, policy.DeltaP2*100);
            title('b) Price Impact Brown Asset [%]', 'FontWeight', 'normal');
            set(p1(1),'Color', color.green, 'LineStyle','-', 'LineWidth', width);
            set(p1(2),'Color', color.middle, 'LineStyle','-', 'LineWidth', width);
            set(p1(3),'Color', color.brown, 'LineStyle','-', 'LineWidth', width);
           % axis([0 4 0 max(max(policy2015.r))*1.20+10^(-15)])
            ...
            subplot(2,3,4)
            p1=plot(x_axis, policy.DeltaPDR1*100);
            title('d) Impact Green PDR [%]', 'FontWeight', 'normal');
            set(p1(1),'Color', color.green, 'LineStyle','-', 'LineWidth', width);
            set(p1(2),'Color', color.middle, 'LineStyle','-', 'LineWidth', width);
            set(p1(3),'Color', color.brown, 'LineStyle','-', 'LineWidth', width);
          %  axis([0 4 min(min(policy2015.c))*0.95 max(max(policy2015.c))*1.05])
            subplot(2,3,5)
            p1=plot(x_axis, policy.DeltaPDR2*100);
            title('e) Impact Brown PDR [%]', 'FontWeight', 'normal');
         set(p1(1),'Color', color.green, 'LineStyle','-', 'LineWidth', width);
            set(p1(2),'Color', color.middle, 'LineStyle','-', 'LineWidth', width);
            set(p1(3),'Color', color.brown, 'LineStyle','-', 'LineWidth', width);
          %  axis([0 4 0 max(max(policy2015.r))*1.20+10^(-15)])
                  ...
                  subplot(2,3,3)
            p1=plot(x_axis, policy.DeltaRf*100);
            title('c) Impact Risk-free Rate [%]', 'FontWeight', 'normal');
           set(p1(1),'Color', color.green, 'LineStyle','-', 'LineWidth', width);
            set(p1(2),'Color', color.middle, 'LineStyle','-', 'LineWidth', width);
            set(p1(3),'Color', color.brown, 'LineStyle','-', 'LineWidth', width);
          %  axis([0 4 min(min(policy2015.c))*0.95 max(max(policy2015.c))*1.05])
