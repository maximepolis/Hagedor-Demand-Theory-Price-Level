function foc_i1=foc_i1(V,Vs,Vtau,s,tau,i1,time,policy_counter,tipping_counter,breakthrough_counter)
...
if breakthrough_counter==1
    ...
    z1=(pref.kappa(1,1)./pref.kappa(2,1)./(production.b(1,time)*cost_scaling(breakthrough_counter,s)))^(1/(pref.rho(1)-1)).*(production.b(2)-(Vtau.*(1-s).*temperature.p(time)*temperature.tcre*tcre_scaling(tipping_counter))./(((1-pref.gamma).*V-Vs.*s).*(1-production.phi(1).*i1))).^(1/(pref.rho(1)-1));
    z2=(pref.kappa(1,2)./pref.kappa(2,2)./(production.b(1,time)*cost_scaling(breakthrough_counter,s)))^(1/(pref.rho(2)-1)).*(production.b(2)-(Vtau.*s.*temperature.p(time)*temperature.tcre*tcre_scaling(tipping_counter))./(((1-pref.gamma).*V-Vs.*s).*(1-production.phi(1).*i1))).^(1/(pref.rho(2)-1));
